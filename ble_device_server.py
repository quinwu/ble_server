...
class BLEDeviceServer:
    ...
    def __init__(self, config_dir: str = "/var/lib/ble_device"):
        ...
        self.wifi_manager = WiFiManager(
            state_callback=self._on_wifi_state_change, status_report_callback=self._on_wifi_status_report
        )
    ...
    def _on_cloud_config(self, upload_url: str):
        ...
        try:
            ...
            # 通知成功
            self._notify_status({"event": "cloud_config_success", "upload_url": upload_url})
    ...
    def _on_capture_command(self, file_batch: str = "", authorization: str = ""):
        ...
        if not self.state_machine.is_ready_for_capture():
            ...
            self._notify_error(
                "capture_not_ready",
                f"WiFi: {self.state_machine.wifi_state.value}, Device: {self.state_machine.device_state.value}",
            )
            return
    ...
    def _do_capture(self, file_batch: str = "", authorization: str = ""):
        try:
            ...
            results = self._capture_multiple_devices(devices=devices, timestamp=timestamp)
            ...
            if success_uploads > 0:
                ...
                self._notify_status(
                    {"event": "upload_success", "success_count": success_uploads, "total_count": len(upload_results)}
                )
            else:
        ...
        for dev in video_devices:
            ...
            try:
                result = subprocess.run(
                    ["v4l2-ctl", "-d", dev_str, "--info"], capture_output=True, text=True, timeout=3
                )
    ...
    def _capture_multiple_devices(self, devices: List[str], timestamp: str) -> Dict[str, Optional[str]]:
        """
        同时从多个设备拍摄照片
    ...
    def _notify_error(self, error_code: str, error_message: str):
        self._notify_status({"event": "error", "error_code": error_code, "error_message": error_message})
