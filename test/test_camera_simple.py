...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='设备类型过滤（如 "USB Camera"），只测试匹配的设备'
    )
    ...
    for dev in devices_to_test:
        ...
        if is_mplane:
            ...
            if subprocess.run(["which", "fswebcam"], capture_output=True).returncode == 0:
                print("\n尝试方法: fswebcam")
                fsweb_methods = [
                    (["-d", actual_dev_str, "-r", "800x600", "--no-banner", "--no-input", "--jpeg", "95", "-F", "5", test_image], "with --no-input"),
                    (["-d", actual_dev_str, "-r", "800x600", "--no-banner", "--jpeg", "95", "-F", "5", test_image], "standard"),
                    (["-d", actual_dev_str, "--no-banner", "--no-input", "--jpeg", "95", "-F", "5", test_image], "auto resolution"),
                ]
                
                for fsweb_cmd, desc in fsweb_methods:
                    fsweb_result = subprocess.run(
                        ["fswebcam"] + fsweb_cmd,
                        capture_output=True,
                        text=True,
                        timeout=10
            ...
            if subprocess.run(["which", "ffmpeg"], capture_output=True).returncode == 0:
                print("\n尝试方法: ffmpeg")
                ffmpeg_methods = [
                    (["ffmpeg", "-f", "v4l2", "-input_format", "uyvy422", "-video_size", "800x600", 
                      "-i", actual_dev_str, "-frames:v", "1", "-y", test_image], "actual device, uyvy422"),
                    (["ffmpeg", "-f", "v4l2", "-video_size", "800x600", 
                      "-i", actual_dev_str, "-frames:v", "1", "-y", test_image], "actual device, auto format"),
                    (["ffmpeg", "-f", "v4l2", "-input_format", "uyvy422", "-video_size", "800x600", 
                      "-i", dev_str, "-frames:v", "1", "-y", test_image], "symlink, uyvy422"),
                ]
                ...
                use_libv4l2 = False
                if libv4l2_path:
                    ffmpeg_methods.insert(0, (
                        ["ffmpeg", "-f", "v4l2", "-input_format", "uyvy422", "-video_size", "800x600", 
                         "-i", actual_dev_str, "-frames:v", "1", "-y", test_image],
                ...
                for idx, (ffmpeg_cmd, desc) in enumerate(ffmpeg_methods):
                    ...
                    if Path(test_image).exists() and Path(test_image).stat().st_size > 0:
                        ...
                        # 过滤掉版本信息
                        error_lines = [l for l in ffmpeg_result.stderr.split('\n') 
            ...
            for capture_cmd, desc in capture_methods:
                ...
                try:
                    capture_result = subprocess.run(
                        ["v4l2-ctl", "-d", actual_dev_str] + capture_cmd,
                        capture_output=True,
                        text=True,
                        timeout=10
                    )
                    ...
                    if test_image_path.exists():
                        try:
                            ...
                            if size > 0:
                                ...
                                print(f"  文件保存在: {test_image}")
                                success_list.append({"capture_cmd": capture_cmd, "desc": desc, "test_image": test_image})
                                break
                            else:
                                print(f"✗ 文件大小为 0")
        ...
        if is_mplane:
            ...
            print("3. 安装 GStreamer: sudo apt-get install gstreamer1.0-tools gstreamer1.0-plugins-base gstreamer1.0-plugins-good")
