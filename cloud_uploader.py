...
class CloudUploader:
    ...
    def __init__(self, upload_url: str = "", timeout: int = 30):
        ...
        self.session.headers.update({"User-Agent": "BLE-Device-Uploader/1.0"})
    ...
    def upload(self, metadata: Dict, authorization: str = "", retry_times: int = 3, retry_delay: int = 2) -> bool:
        """
        上传数据到云端（标准 POST 请求）
        ...
        for attempt in range(retry_times):
            try:
                # 设置请求头
                headers = {"Content-Type": "application/json", "User-Agent": "BLE-Device-Uploader/1.0"}
                ...
                response = self.session.post(self.upload_url, json=metadata, headers=headers, timeout=self.timeout)
        ...
        try:
            with open(file_path, "rb") as f:
                for chunk_index in range(total_chunks):
                    ...
                    # 上传分片
                    success = self._upload_chunk(chunk_data, chunk_index, total_chunks, file_path.name, retry_times)
    ...
    def _upload_chunk(
        self, chunk_data: bytes, chunk_index: int, total_chunks: int, filename: str, retry_times: int
    ) -> bool:
        """上传单个分片"""
        for attempt in range(retry_times):
            try:
    ...
    def test_connection(self) -> bool:
        ...
        try:
            # 发送 HEAD 请求测试连接
            response = self.session.head(self.upload_url, timeout=10)
